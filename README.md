...
八档只是默认的自动分流策略，不是模型数量限制。可以在右侧“模型与自动分流”中把档位数量调整为 2～12 档，编辑每档名称和判断描述、调整顺序，并选择执行模型与推理强度；结果写入 Git 忽略的 `router.config.local.json`，重启后继续生效。也可以直接修改 `router.config.json` 的公开默认值。手动模式通过 Codex `model/list` 实时读取账号里的全部可见模型及其推理强度；指定模型不存在或强度不受支持时会明确报错。分类模型会同时读取 `model/list` 返回的模型官方简介、支持的推理强度和当前档位规则；不会为每次提问重新访问网页。自动分类会产生额外模型用量，不保证每次都比手动选择节省。
