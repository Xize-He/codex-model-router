...
3. 输入问题，默认自动分流；下拉框可选择账号当前返回的全部模型和推理强度。左侧“联网搜索”可在新对话开始前选择自动、开启或关闭。Enter 发送，Shift + Enter 换行。
...
联网搜索直接使用 Codex 原生 Web Search，不依赖 MCP，也不启用浏览器自动化。“自动”对应 Codex 的 `cached` 默认模式，由模型按任务决定是否检索 OpenAI 维护的搜索索引；“开启”对应 `live`，允许实时外部网页检索；“关闭”对应 `disabled`，从会话中移除搜索工具。搜索模式在创建或首次恢复 Codex 会话时确定，因此已经开始的会话只显示其既定模式；要更换模式请新建对话。搜索行为和可用性仍受当前账号及管理员策略限制。
