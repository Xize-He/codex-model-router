...
DeepSeek API 独立计费，不使用 ChatGPT 套餐额度；账户用量面板仍展示 Codex 账户额度。创建会话时选择 **DeepSeek · Harness** 使用官方 DeepSeek Harness。现有 Codex 登录保留；切换引擎请新建对话。

模型列表检测只验证密钥和模型可用性，实际生成仍受提供商余额与服务状态影响。
