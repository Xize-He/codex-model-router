...
3. 输入问题，默认自动分流；下拉框可选择账号当前返回的全部模型和推理强度。Enter 发送，Shift + Enter 换行。
...
八档只是默认的自动分流策略，不是模型数量限制。可以在右侧“模型与自动分流”中选择判断模型、推理强度及每一档的路由；选择结果写入 Git 忽略的 `router.config.local.json`，重启后继续生效。也可以直接修改 `router.config.json` 的公开默认值。手动模式通过 Codex `model/list` 实时读取账号里的全部可见模型及其推理强度；指定模型不存在或强度不受支持时会明确报错。GPT-5.6-Sol 分类时会同时读取 `model/list` 返回的模型官方简介、支持的推理强度和本地八档规则；不会为每次提问重新访问网页。自动分类会产生额外模型用量，不保证每次都比手动选择节省。
