...
3. 输入问题，默认自动分流；下拉框可选择账号当前返回的全部模型和推理强度。左侧“联网搜索”可随时选择默认（缓存）、开启或关闭，当前对话从下一条消息开始使用新设置。Enter 发送，Shift + Enter 换行。
...
联网搜索直接使用 Codex 原生 Web Search，不依赖 MCP，也不启用浏览器自动化。“默认（缓存）”对应 Codex 的 `cached` 默认模式，由模型按任务决定是否检索 OpenAI 维护的搜索索引；“开启”对应 `live`，允许实时外部网页检索；“关闭”对应 `disabled`，从对话中移除搜索工具。选择会保存在当前浏览器中作为工作台默认值；已经开始的对话会在下一条消息前通过 Codex 原生恢复接口重新应用设置，历史和对话 ID 保持不变。搜索行为和可用性仍受当前账号及管理员策略限制。
